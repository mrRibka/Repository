public interface ConstantString {
    String INPUT_STRING_DATA = "input.string.data";
    String WRONG_INPUT_DATA = "input.wrong.data";
    String FIRST_NAME = "input.data.first.name";
    String LAST_NAME = "input.data.last.name";
    String PATRONYMIC = "input.data.patronymic";
    String NICK = "input.data.nick";
    String COMMENT = "input.data.comment";
    String GROUP = "input.data.group";
    String FIRST_MOBILE_PHONE = "input.data.first.mobPhone";
    String E_MAIL = "input.data.e-mail";
    String SKYPE = "input.data.skype";


    String REGEX_NAME = "regex.name.regexp";
    String REGEX_LAST_NAME = "regex.last.name";
    String REGEX_PATRONYMIC = "regex.patronymic";
    String REGEX_NICK = "regex.nick";
    String REGEX_COMMENT = "regex.comment";
    String REGEX_GROUP = "regex.group";
    String REGEX_FIRST_MOBILE_PHONE = "regex.first.mobPhone";
    String REGEX_SECOND_MOBILE_PHONE = "regex.second.MobPhone";
    String REGEX_HOME_PHONE = "regex.homePhone";
    String REGEX_EMAIL = "regex.e-mail";
    String REGEX_SKYPE = "regex.skype";
    String REGEX_ADDRESS_INDEX = "regex.address.index";
    String REGEX_ADDRESS_CITY = "regex.address.city";
    String REGEX_ADDRESS_STREET = "regex.address.street";
    String REGEX_ADDRESS_HOUSE_NUM = "regex.address.houseNum";
    String REGEX_ADDRESS_FLAT_NUM = "regex.address.flatNum";
}
